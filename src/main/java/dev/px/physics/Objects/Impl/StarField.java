...
public class StarField implements Renderable {
    ...
    public StarField(int numStars, double radius) {
        this(numStars, radius, null);
    }

    public StarField(int numStars, double radius, ScalingSystem scalingSystem) {
    ...
    public void render(double mouseX, double mouseY, double delta) {
        ...
        double scale = scalingSystem != null ? scalingSystem.getScale() : 1.0;

        for (Star star : stars) {
            float bright = (float) star.brightness;
            GL11.glColor3f(bright, bright, bright);
            GL11.glVertex3d(star.x * scale, star.y * scale, star.z * scale);
        }
